var Splitiscope = function (config) {
    ...
    var side_effects = d3.dispatch.apply(this, d3.keys(__))
        ...
        .on("data", function () {
            ...
            parseData();
            console.log("splitscope: data loaded");
        })
    ...
    splitiscope.toString = function () {
        return "Splitiscope: (" + d3.keys(__.data[0]).length + " total) , " + __.data.length + " rows";
    };
    ...
    function drawMultipleBarchart(data_points) {
        ...
        function text_color(point) {
            return addOffset(point) ? "#fff" : null;
        }

        function addOffset(point) {
            return ( vertical_offset(point) > band / 2);
        }
        ...
        function text_styling(selector, point) {
            selector
                ...
                .style("visibility", function (point) {
                ...
                if (!_.isNull(split_data["x"].span)) {
                    ...
                    if (__.dataType.x === "n") {
                        ...
                        var xExtent = scales.x.range(),
                            xSelectedVals = _.filter(xExtent, function (val) {
                ...
                if (!_.isNull(split_data["y"].span)) {
                    ...
                    if (__.dataType.y === "n") {
                        ...
                        var yExtent = scales.y.range(),
                            ySelectedVals = yExtent.filter(function (val) {
    ...
    function removeCategoricalSplitValue(value, axis) {
        ...
        var remaining_values = len ? _.difference(scales[axis].domain(), selected[axis]) : scales[axis].domain(),
    ...
    function setClassScales() {
        ...
        __.categoryColor = numberOfCategories ? d3.scale.ordinal().domain(colorCategories).range(colorArray) : function () {
            return colorArray;
        };
...
//returns an empty array if kde cannot be calculated
    function sampleEstimates(kde, range) {
        ...
        var newPoints = d3.range(range[0], range[1], stepSize);
